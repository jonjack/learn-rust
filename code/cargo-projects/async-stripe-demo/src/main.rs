...
async fn main() {
    ...
    let client = Client::new(grazie_key);
    ...
    */
    ...
    // Debugging function to print an object's type
    ...
    let payment_intent = {
        ...
        create_intent.payment_method_types = Some(vec!["card".to_string(), "link".to_string(), "klarna".to_string()]);
    ...
    let secret = payment_intent.capture_method;
    println!("secret{}: ", secret);


    print_type_of(&payment_intent.capture_method);
    print_type_of(&payment_intent.client_secret);

    println!("foo {}", payment_intent.client_secret.as_deref().unwrap_or("none-case string"));


    println!("{}", serde_json::to_string_pretty(&payment_intent).unwrap());
